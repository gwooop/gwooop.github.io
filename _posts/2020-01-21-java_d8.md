...
# Medici Education Java Day8

## Table of Contents
  71. [함수형 프로그래밍](#char-admin)
  72. [Java8에서부터 함수형 프로그래밍 지원](#java8에서부터-함수형-프로그래밍-지원)
  73. [람다식](#람다식)
  74. [람다식-기본문법](#람다식-기본문법)
  75. [타겟-타입](#타겟-타입)
  76. [함수형-인터페이스](#함수형-인터페이스)
  77. [제네릭](#제네릭)
  78. [제네릭 타입](#제네릭-타입)
...
- 대용량 데이터 처리에 유리하다.  
  - 1) 데이터를 포장해서 객체를 생성하는 것 보다는 데이터를 바로 처리하는 것이 속도에 유리하다.  
  - 2) 멀티코어 CPU에서 데이터를 병렬 처리하고 취합할 때 객체보다는 함수가 유리함.  
- 이벤트 지향 프로그래밍  
...
자바에서 람다식을 수용한 이유  
- 코드가 매우 간결하다.  
- 컬렉션 요소(대용량 데이터)0를 필터링 또는 매핑해서 쉽게 집계할 수 있다.  
...
```
파라미터 타입은 런타임시에 대입값에 다라 자동으로 인식하기 때문에 생략 가능   
```java
   (a) -> {Sytem.out.println(a);}
```
...
이 어노테이션을 사용하면 체크를 해줄뿐 하나의 메소드를 가진 인터페이스가 있으면  
함수형 인터페이스라고 보면된다.  
